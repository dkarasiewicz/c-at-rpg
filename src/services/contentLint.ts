/**
 * Hard mechanical constraint lints for DM-generated content (pure functions,
 * unit-tested in tests/gm.spec.ts).
 *
 * These ran server-side inside `api/gm/*` until the endpoints were retired.
 * They now run in the two places that actually consume generated content:
 * the browser (`services/dm.ts`, before a generated party is allowed near the
 * engine) and the agent (`agent/tools/contribute_content.ts`, before anything
 * reaches the shared pool). Same functions, same numbers, no copies.
 *
 * Numbers are anchored to the shipped content and live in `./caps.ts`:
 ...
 *    content budget), costs 0..6, Σ non-basic costs <= 16, power caps from
 *    the §4 reference table;
 ...
 */
import type { GameEvent, Rarity, Skill } from "../core/types.js";
...
import type { GeneratedCatKit, GeneratedEquip } from "./gmTypes.js";
import {
  CLASS_IDS,
  EVENT_CAPS,
  GM_ROLES,
