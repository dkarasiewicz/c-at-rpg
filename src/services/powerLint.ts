/**
 * Power Script validation and budget stamping for DM-authored powers
 * (stand-powers.md §Balance: "the lint runs at compile time AND client-side
 * at battle setup").
 *
 * This ran inside `api/_lib/powers.ts` until the `api/gm/*` endpoints were
 * retired; it now lives in the ui-side services package, where the browser
 * runs it on everything the DM sends back. The budget pricing is NOT
 * duplicated here: `powerBudget` and `validatePowerScript` are the canonical
 * pure functions from `src/core/combat/powers.ts`. This module adds only the
 * wrapping: error-string lints, budget stamping (models never compute their
 * own budgets), the canonical resonance pair key, stock fallback powers, and
 * the JSON Schemas the one-shot DM calls pass as `outputSchema`.
 */
import {
  BUDGET_CAPS,
  EFFECT_CAPS,
...
import { STATUS_IDS } from "./caps.js";
